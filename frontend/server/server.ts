...
import { MinioClientWrapper } from "./minio-client";
...
const healthzHandler = async (_, res) => {
  try {
    const response = await fetch(
      `${apiServerAddress}/${v1beta1Prefix}/healthz`,
      { timeout: 1000 }
    );
...
const artifactsHandler = async (req, res) => {
  const [source, bucket, encodedKey] = [
    req.query.source,
    req.query.bucket,
    req.query.key
  ];
      ...
      try {
        ...
        matchingFiles.forEach((f, i) => {
          ...
          f.createReadStream()
            ...
            .on("error", () =>
              res.status(500).send("Failed to read file: " + f.name)
            );
        });
      } catch (err) {
      ...
      minioClient.getObject(bucket, key, (err, stream) => {
        if (err) {
          res
            .status(500)
            .send(
              `Failed to get object in bucket ${bucket} at path ${key}: ${err}`
            );
          return;
        }
        ...
        try {
          let contents = "";
          const tarStream = stream.pipe(new tar.Parse());
          tarStream.on("entry", (entry: Stream) => {
            entry.on("data", buffer => {
              contents += buffer.toString();
              console.log(contents);
            });
          });
          tarStream.on("error", function(err: any) {
            res
              .status(500)
              .send(
                `Failed to get object in bucket ${bucket} at path ${key}: ${err}`
  ...
  proxy({
    ...
    pathRewrite: path =>
      path.startsWith(BASEPATH)
        ? path.substr(BASEPATH.length, path.length)
        : path,
    target: apiServerAddress
  })
);
